...
export default class Calendar extends React.Component {
  constructor(props) {
    ...
    this._updateInputProps(
      this.props.startDateInputProps,
      this.props.endDateInputProps
    );
  ...
  _renderActions() {
    ...
    return (
      <div className="tsc-calendar__actions">
        <div
          className="tsc-calendar__action tsc-calendar__action-element tsc-calendar__action-element--left"
          onClick={this._onGoToPrevMonth.bind(this)}
        >
          &#8249;
        </div>
        ...
        <div
          className="tsc-calendar__action tsc-calendar__action-element tsc-calendar__action-element--right"
          onClick={this._onGoToNextMonth.bind(this)}
        >
          &#8250;
        </div>
      </div>
  ...
  _renderMonth() {
    const { currentDate, selectedTimeslots } = this.state;

    const { timeslots, initialDate } = this.props;
  ...
  _renderInputs() {
    ...
    const { startDate, endDate } = this.inputProps;
  ...
  _formatDisabledTimeslots() {
    const { disabledTimeslots } = this.props;

    return disabledTimeslots.map((timeslot) => {
      let timeslotMoment = Object.assign({}, timeslot);
      timeslotMoment.startDate = moment(
        timeslotMoment.startDate,
        timeslotMoment.format
      );
      timeslotMoment.endDate = moment(
        timeslotMoment.endDate,
        timeslotMoment.format
      );
  ...
  _onTimeslotClick(newTimeslot) {
    ...
    const { maxTimeslots, onSelectTimeslot } = this.props;
